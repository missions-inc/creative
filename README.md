...
### ✅ 公開後の改善（UI/UX 改修）
- **色分けルールの集約**（`lib/tasks/colors.ts`）: 枠線＝期日の緊急度（超過=赤/2日以内=黄）、
  バッジ＝ステータス（未着手=赤/進行中=黄/確認待ち=緑/完了=グレー）。テキストラベル併記。
  緊急度判定は `filters.ts`（リマインド通知と同一定義）へ委譲
- **グローバルメニュー**: 「ダッシュボード」がホバー/キーボードで開くドロップダウンになり、
  期日間近・マイタスク・クライアント別を直接開ける（`/dashboard?tab=...`）
- **ダッシュボード**: 完了タスクを全タブで非表示（完了はプロジェクト詳細の折りたたみでのみ表示）。
  マイタスクを期日間近と同じ3分類（超過→2日以内→それ以外）に統一。
  期日間近・クライアント別タブに担当者絞り込み（OR条件・候補は表示中タスクの担当者のみ・
  タブ間で選択保持）。右上の「タスクを追加」からモーダルで作成でき、
  プロジェクト選択欄の「＋新規作成」（PM以上）でその場でプロジェクトも作れる
  （クライアント新規登録は admin のみ＝既存権限どおり）
- **プロジェクトページ**: クライアント名を大きく＋緑マーカーで階層を明示
  （クライアント別タブも同様）。完了タスクは下部の折りたたみに集約
